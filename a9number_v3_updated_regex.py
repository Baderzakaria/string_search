...
"""
# Define punctuation characters to remove, excluding apostrophes and considering hyphens and single quotes as word characters

def count_occurrences_in_text(word, text):
  """
  Counts occurrences of a word (case-insensitive) in text using regex.
  """
  if word == "maley":
    text = text.replace("'", "")
  pattern = rf"\b{word.lower()}(?!')\b" # Word boundary + word (lowercase) + word boundary
  matches = re.findall(pattern, text, flags=re.IGNORECASE)  # Find all matches (case-insensitive)
  return len(matches)
