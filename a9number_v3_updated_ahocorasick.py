...
def build_automaton(patterns):
    """
    Build a pyahocorasick automaton from a list of patterns (words).
    """
    automaton = ahocorasick.Automaton()
    padding_start = [" ", "\n", "\t"] + list(string.punctuation)
    padding_end = [" ", ".", ";", ",", "-", "–", "—", "?", "!", "\n"] + list(string.punctuation)
    for pattern in patterns:
        for start_padding in padding_start:
            for end_padding in padding_end:
                padded_pattern = start_padding + pattern + end_padding
                automaton.add_word(padded_pattern, pattern)
...
def count_occurrences_in_text(word, text):
    ...
    Counts words that are standalone, at the beginning or end of a sentence, or adjacent to punctuation.
    """
    normalized_text = preprocess_text(text)
    normalized_word = word.lower()
    padded_text = " " + normalized_text + " "
    automaton = build_automaton([normalized_word])
    occurrences = {normalized_word: 0}
    for match in automaton.iter(padded_text):
        end_index, pattern = match
        occurrences[pattern] += 1
    return occurrences[normalized_word]
